...
public interface TrendsRepo extends JpaRepository<Trends_Bean, Long> {
	...
	@Query(value = "SELECT itemname FROM trends WHERE rest_id=?1 and date=?2 and itemtype=?3 ORDER BY id", nativeQuery = true)
	List<String> RetrieveItemNameByDate(long rest_id, int date, int itemtype);
	
	@Query(value = "SELECT itemname FROM trends WHERE rest_id=?1 and date>=?2 and date<=?3 and itemtype=?4 ORDER BY id", nativeQuery = true)
